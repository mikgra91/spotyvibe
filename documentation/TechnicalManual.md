...
- **Track list (Discover)** — generated tracks appear inside the Discover Music section, below the Generate button, separated by an `<hr class="inline-divider">`. The `#discoverTrackArea` wrapper is hidden when empty and revealed by `renderTracks()` or `showStatus()`/`showPlaylistLink()` for terminal events. Displays suggestions with album cover thumbnails, a **Feedback** button (opens the dual-submit reason panel) and a **Delete** button (🗑, removes without recording feedback). Track cards glow green on hover via `box-shadow`.
- **Refine Playlist** — collapsible section with a playlist dropdown (lazy-loaded on first expand via `populateReviewPlaylistPicker()`), a "Load Playlist" button with an inline loading spinner, and a review track list inside the section. The `#reviewTrackArea` wrapper is hidden until tracks are loaded. Each track card supports **Feedback** (dual-submit panel: Like stays in playlist, Dislike removes from playlist) and **Delete** (🗑, removes from playlist without recording feedback). Track rows render via the shared `buildTrackCardHtml()` helper in `frontend/static/js/modules/feedback.js`; dispatch of `toggleFeedback` / `toggleReviewFeedback` is via onclick attributes with no polarity argument — polarity is chosen at submit (`submitFeedback(idx, 'like'|'dislike')`).
- **Preview overlay** — bottom-sheet three-zone layout: (1) **player** (centered, responsive width 50vw / min 420px / max 700px) — custom Web Playback SDK panel (`#sdkPlayer`) with album art, transport, scrubber, and 👍 / 👎 quick buttons that submit immediately for Premium accounts on Widevine/FairPlay-capable runtimes, or the iframe embed (`#spotifyPreviewIframe`, ~30-second previews) as fallback; (2) **action buttons** — **Feedback** (`#previewTabFeedback`, opens `#previewFeedbackPanel`) and **Delete** (🗑, calls `previewDismiss()`); (3) **sliding feedback panel** — artist/track/reason inputs with two submit buttons at the bottom (`#previewFbSubmitLike` green / `#previewFbSubmitDislike` red) plus a Cancel button. `openPreviewFeedbackPanel()` opens the panel without a polarity pre-selection; `submitPreviewFeedback(action)` receives polarity from the clicked submit button. Quick 👍 / 👎 in the SDK player are wired to `quickLike()` / `quickDislike()` which call `_postFeedbackAndReact(action, …, null)` for immediate submit. On the first preview open per device (`localStorage.spv_preview_rate_hint_seen`), the player's quick buttons pulse via the `.sdk-quick.pulse` keyframe animation and `tips.js` surfaces the `first_preview_open` entry (`preview.rate_hint` i18n). Embed URLs include a `_cb=<timestamp>` cache-bust parameter to avoid caching stale iframe state. SDK route: `core/src/playlist.py::get_spotify_access_token()` and `get_spotify_session_info()` back `/api/spotify/token` and `/api/session`; `frontend/static/js/modules/spotify-sdk.js` wraps the SDK lifecycle.
- **Feedback form** — expandable per-track form with artist, track, and reason fields. The form-actions row uses the `.form-actions-dual` class — two submit buttons (`.btn-submit-like` green, `.btn-submit-dislike` red) plus Cancel — so polarity is picked at submit time. In the preview overlay, it slides in from the right as part of the three-zone layout.
