...
- **AI生成音楽をフィルタ** *(任意)*  
  有効にすると、SpotyVibe は提案された楽曲のうち、コミュニティが管理するAI生成アーティストのブロックリストに載っているアーティストの曲を除外します（Spotify のアーティスト ID で照合）。デフォルトは無効です。先にブロックリストをダウンロードする必要があります — このオプションの下に表示される **今すぐダウンロード** ボタンを使ってください。インストール後はその行にインストール済みバージョンが表示され、ボタンは **更新を確認** に変わります。ブロックリストは候補プールのコーパスとは独立して更新されるため、コーパスの再構築を待つ必要はありません。データはオープンソースの [spotify-ai-blocker](https://github.com/CennoxX/spotify-ai-blocker) プロジェクト（MIT）から取得しています。
