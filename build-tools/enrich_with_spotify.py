...
from spotify_enrichment.client import SpotifyClient  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Pass 1: search every MB artist, score, and remember the candidate ID.
    # We don't write anything yet — we want to batch the detail fetch
    # (50 IDs per call) so we don't burn 1 call per artist.
    pending: list[tuple[dict, str]] = []   # (mb_row, matched_spotify_id)
    skipped: list[dict] = []               # mb rows we couldn't match
    n_seen = 0
    n_matched = 0
    n_skipped_low_pop = 0
    started = time.time()

    logger.info("Scanning input %s …", args.input)
    for mb_row in _iter_input(args.input):
        n_seen += 1
    ...
    elapsed = time.time() - started
    logger.info(
        "Done: %d total artists written, %d enriched (%.1f%%), %d untouched in %.1f min",
        n_seen, n_enriched, 100.0 * n_enriched / max(1, n_seen),
        n_seen - n_enriched, elapsed / 60.0,
    )
    return 0
