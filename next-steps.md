...
### Corpus probe — track-level data now baked into the corpus ✅

The Google Cloud Run job was modified to fetch top tracks per artist
alongside the existing Last.fm enrichment. The rebuilt corpus was
downloaded to `%LOCALAPPDATA%/spotyvibe/rag_corpus/artists.jsonl.gz`
(22 MB, ~70 % larger than the 2026-05-06 Last.fm-only build).

**Manifest:** `corpus_version=2026-05-19`, built `2026-05-19T01:25:04Z`,
sha256 `9935cc21…`. 175,578 artists loaded by `RagCorpus.load()` in ~7 s.

**`top_tracks` coverage:** **146,515 / 175,578 = 83.4 %** of artists
now carry a baked-in top-tracks list (typically 5 titles each). Real
Spotify-resolvable titles verified end-to-end:

| Artist | Sample `top_tracks` |
|---|---|
| The Beatles | Here Comes the Sun – Remastered 2009, Hey Jude, Eleanor Rigby, … |
| Metallica | Enter Sandman, Nothing Else Matters, Master of Puppets, … |
| Aphex Twin | Avril 14th, Xtal, Pulsewidth, Ageispolis, i |
| SB19 | Gento, MAPA, I WANT YOU, … |
| Charlotte Sands | Dress, … |

`OverlayVerifier` smoke test against the live corpus resolved 8/10
synthetic queries correctly (mainstream + niche), with the two
intended `not_found`s for a nonexistent track and a nonexistent artist.

### OverlayVerifier substring-match bug (surfaced + fixed) ✅

The smoke test caught a real false positive that did not exist before
the cloud rebuild: `Aphex Twin / Windowlicker` resolved as `found`
because the corpus legitimately stores a 1-char track title `"i"`
(real Aphex Twin track on *Selected Ambient Works 85-92*) and the
substring rule `n_ot in q` accepted `'i' ⊂ 'windowlicker'`. With
146 k overlay rows now in play, short titles like `"i"`, `"go"`,
`"on"` would inflate found-rates on every L0 / `l0_l1` eval run.

**Fix:** [`OverlayVerifier.verify`](core/src/verify.py#L189) now
requires `min(len(n_ot), len(q)) >= 4` for the substring rule;
equality still matches short titles unconditionally. Two regression
tests added in
[`TestOverlayVerifier`](core/tests/test_verify.py#L257) cover both
substring directions (short overlay ⊂ long query, short query ⊂ long
overlay). All 47 verify tests green.

Post-fix re-check: Windowlicker correctly `not_found`; exact-match
on `"I"` still resolves; remaster-suffix `Here Comes the Sun →
Here Comes the Sun - Remastered 2009` unchanged.

### Status updates on tasks that were waiting for richer overlay coverage

- **N4 — Multi-scenario overlay rebuild → OBSOLETE.** The cloud-side
  fix delivered 83.4 % coverage across the entire 175 k corpus in one
  build, completely superseding the per-scenario
  `build_top_tracks_overlay.py --scenarios all --resume` pass. The
  separate `top_tracks_overlay.json` file (27 KB, dated 2026-04-27)
  is now dead weight — `RagCorpus.load()` merges it after the JSONL
  rows, but every artist it covers is already populated from the
  corpus itself, so the merge is a no-op. **Follow-up:** consider
  deleting the overlay auto-detect path in
  [`core/src/rag/corpus.py:186-210`](core/src/rag/corpus.py#L186-L210)
  and the standalone `build_top_tracks_overlay.py` script after one
  more corpus build confirms coverage doesn't regress.
- **Phase 2 — Track A Step 4 (S.6 #4 "overlay rebuild script execution
  deferred") → OBSOLETE.** Same reason as N4. The script ships and
  works, but there is no longer a reason to run it.
- **R1.2 deferred prompt change ("ALWAYS OMIT no-known artists") →
  UNBLOCKED.** Previously rejected because overlay coverage was
  default-scenario-only; with 83.4 % global coverage the
  `single_artist_no_known` failure mode is now rare. The R1.2 strict
  rule can be re-attempted. Still requires a paid eval run to
  validate (probe gate via B-11 should be run first — costs ≈ $0.01).
- **Track A Step 7 (verify-mode `spotify` vs `l0_l1` side-by-side
  baseline) — still pending.** Now far more interesting because L0
  hit rate on niche scenarios should be dramatically higher. Still
  blocked on Spotify OAuth cache (OP2). No code change needed.
- **A4 (Spotify-resolvability collapse on `lastfm_tag_weighting`,
  E7 found-rate 7-27 %) — partially mitigated.** The Stage 3 prompt
  already biases toward overlay tracks (`known:` examples); with
  83.4 % global coverage the share of pool members that present a
  `known:` line jumps materially. A re-baseline of E7
  `lastfm_tag_weighting` would quantify the lift. Still requires
  paid Spotify eval — user-triggered.

### Recommended next user action (paid)

When the user is ready to spend the eval budget, the highest-signal
run on the new corpus is:

```bash
# Cheap (~$0.01) — verify the corpus rebuild flipped B-11's verdict
python -m evaluation.probes --model gpt-5.4-mini --probes B-11 --confirm

# Medium (~$0.40) — re-baseline R1 prompt set on richer corpus
python evaluation/run_evaluation.py --verify-mode l0_l1 \
    --scenarios default,lastfm_tag_weighting --iterations 3
```

The first burns no Spotify quota; the second uses the new `l0_l1`
chain (Overlay → MusicBrainz → Last.fm), so Spotify is touched only
for an optional ground-truth pass.
...
**Scope:** 2 scenarios × 1 iter × 1 model, `--verify-mode spotify`.
Wall 24 min, cost **$0.024 total**.

### Headline results

| Metric | default | lastfm_tag_weighting | Historical E7 (2026-05-08) |
|---|---:|---:|---|
| Playlist A/B tracks | **15/15 + 15/15** | **15/15 + 15/15** | 30/32 under or empty across all 4 models |
| Spotify-found rate | **96.8 %** | **93.8 %** | mini 40 %, **lastfm_tag_weighting 7-27 %** (A4 trigger) |
| Leakage | 0 / 0 | 0 / 0 | pass |
| Fit-check | 1 decade hit | n/a | similar |
| Cost / playlist | $0.007 | $0.005 | gpt-5.4 $0.08-0.15, mini $0.04 |
| Wall | 253 s | 140 s | Stage 3 alone 30-180 s |

### Status updates triggered by these numbers

- **A4 (Spotify-resolvability collapse on `lastfm_tag_weighting`) →
  RESOLVED.** Spotify-found jumped 7-27 % → **93.8 %** = **+66 to
  +86 pp absolute lift**, on the exact scenario A4 was filed
  against. Root cause was always "Stage 3 picks tracks Spotify
  cannot resolve"; the rebuilt corpus puts 5 already-resolved top
  tracks per artist directly in the RAG row, and Stage 3 now picks
  from a `known:` list that is by construction resolvable.
- **A1 (under-fill cap `MAX_GPT_CALLS_PER_RUN = 4`) → no longer
  binding.** Both playlists hit 15/15 in 4 batches in both
  scenarios. The cap stops mattering once Stage 3 produces
  resolvable tracks. **Do not lift the cap** — the original concern
  is gone, lifting it would just inflate cost on the rare
  pathological model.
- **R1.2 prompt change ("ALWAYS OMIT no-known artists") → still
  worth a retry,** but downgraded in urgency. The empty-pool case
  that historically broke R1.2 is now rare (83.4 % global
  coverage); however the new corpus also makes the rule less
  necessary, since most artists now carry `known:` examples.
  Net-net: low ROI to chase. Park unless a probe shows
  confabulation creeping back.
- **C1 (L5 two-tier Stage 3, gpt-5.4 auto-escalation) → less load-
  bearing.** Cost ceiling target of < $0.10/playlist is **met at
  $0.005-0.007** by DeepSeek V4 Flash alone. The "Auto" preset
  default-flip is no longer required for cost reasons.
- **OPEN — Compare DeepSeek V4 Flash vs gpt-5.4-mini on the new
  corpus.** This run is single-model; the +93.8 % on
  `lastfm_tag_weighting` could be partly model-effect rather than
  pure corpus-effect. A 1-iter `gpt-5.4-mini` run on the same two
  scenarios (~$0.10) would isolate that. Recommended next eval.

### Caveats

- **n=1.** B-6 variance work showed `n_required_for_5pp_signal ≥
  5` even on stable models. The corpus-rebuild *direction* is far
  above noise (+66 pp absolute), but absolute cite-rates etc. are
  not design-quality numbers yet.
- **DeepSeek V4 Flash baseline is new.** Cite-rate 65.6 % on
  `lastfm_tag_weighting` (vs 93.5 % on default) could be model-
  specific instead of corpus-specific — a cross-model run would
  separate the two.
- **`:free` tier failed at run time.** `deepseek/deepseek-v4-
  flash:free` returned 429 "temporarily rate-limited upstream"
  on the profile-train call; aborted before any playlist was
  generated. For reliable evals stick to the paid `:free`-less
  variant or another model.

### Operational notes

- `--release-lock` clears the stale `evaluation/.run.lock` file
  and **exits without running the eval**. Two separate invocations
  are needed: `--release-lock`, then the real run.
- The harness's `OpenRouter routing active` log line confirms the
  `[openrouter] api_key = …` block in `settings.ini` was picked
  up — no env-var wiring required.
- The inter-scenario cooldown is 600 s by default; combined with
  the eval per-scenario time of 140-540 s, a 2-scenario × 1-iter
  run takes ~24 min wall regardless of model speed.

### Recommended next user action (cheap, ~$0.10)

```bash
# Cross-model sanity: confirm the corpus-rebuild lift is corpus-side,
# not DeepSeek-specific. Same scenarios, same verify-mode, n=1.
# Edit evaluation/settings.ini: models = gpt-5.4-mini
# (keep [openrouter] api_key intact — only models field changes)
python evaluation/run_evaluation.py --no-confirm --no-probe-gate \
    --verify-mode spotify
```

If `gpt-5.4-mini` also lands ≥ 90 % Spotify-found on
`lastfm_tag_weighting`, A4 is fully closed and we can move the
agenda on to U3 / Q2 / docs-refresh per the original Post-Phase B
plan.
...
- ⏳ **Track A Step 7 still pending** — `--verify-mode spotify` vs
  `--verify-mode l0_l1` side-by-side baseline on the canonical
  scenario. Still requires Spotify OAuth (the `spotify` arm) which
  is blocked by OP2 (cache disappeared again 2026-05-13). Document
  the offset in `documentation/TechnicalManual.md` once unblocked.
  Estimated cost: 2× normal eval spend (~$0.50 for the four-model
  row).
