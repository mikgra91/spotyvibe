...
- **Reverted `json_schema` Stage 3 wiring** in `core/src/suggestions.py` — Stage 3 is back to plain `{"type": "json_object"}`. The `_STAGE3_JSON_SCHEMA` constant and `_stage3_response_format()` helper were initially kept as available infrastructure but removed 2026-04-30 (S7) as dead code with zero call-sites; recover via git history if a future experiment needs them. `_JSON_SCHEMA_UNSUPPORTED` cache + `_looks_like_schema_rejection()` in `core/src/openai_http.py` are kept (dormant — only activates when a caller passes `json_schema`).
